...
def main():
  ...
  if "linux" in sys.platform:
    ...
    subprocess.check_call("dir bazel_win", shell=True)
    subprocess.check_call("dir C:\\b\\s\\w\\ir\\bazel_win\\bazel.exe", shell=True)
    subprocess.check_call("timeout /t 300", shell=True)
    subprocess.check_call(["C:\\b\\s\\w\\ir\\python\\bin\\python.exe", "--version"])
    subprocess.check_call(["C:\\b\\s\\w\\ir\\bazel_win\\bazel.exe", "version"])
