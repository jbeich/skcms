...
print ("build_or_test: %s" % build_or_test)
print ("local_or_rbe: %s" % local_or_rbe)
...
bazel_cache_dir = tempfile.mkdtemp(prefix="bazel-cache-", dir=os.environ["TMPDIR"])

def bazel(*args):
  cmd = ["bazel", "--output_user_root=" + bazel_cache_dir]
  for arg in args:
    cmd.append(arg)
  call(" ".join(cmd))
