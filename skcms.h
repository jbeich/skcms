...
typedef struct skcms_ICCProfile {
    ...
    // If has_trc, we'll try to approximate each curve with an skcms_TF13.
    bool   has_tf13[3];
    skcms_TF13 tf13[3];
} skcms_ICCProfile;
...
// skcms_Parse() creates a profile that guides skcms_Transform() to favor accuracy.
// If you are willing to trade a small amount accuracy for a significant speedup, call this.
