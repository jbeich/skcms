...
bool skcms_gauss_newton_step(float (*     t)(float x, const void*), const void* t_ctx,
                             ...
                             float P[4],
                             const float x0, const float x1, const float dx) {

    // Let's let N be the number of x values we'll test.
    int N = 0;
    for (float x = x0; x <= x1; x += dx) {
        N++;
    }
    assert (N == (int)( (x1-x0) / dx ));  // TODO: double-check and use this closed-form math?
    ...
    //   Jf     is [N x 4], a matrix with an entry for each (P,x) pair
    ...
    // 4) multiply inverse lhs by rhs
    skcms_Vector4 dP = skcms_Matrix4x4_Vector4_mul(&lhs, &rhs);
    P[0] += dP.vals[0];
    P[1] += dP.vals[1];
    P[2] += dP.vals[2];
    P[3] += dP.vals[3];
