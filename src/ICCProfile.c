...
void skcms_ICCProfile_getCreationDateTime(const skcms_ICCProfile* profile,
                                          skcms_ICCDateTime* dateTime) {
    if (!profile || !profile->buf || !profile->len || !dateTime) { return; }
    const skcms_ICCHeader* header = (const skcms_ICCHeader*)profile->buf;
    dateTime->year   = read_big_u16(header->CreationDateTime + 0);
    dateTime->month  = read_big_u16(header->CreationDateTime + 2);
    dateTime->day    = read_big_u16(header->CreationDateTime + 4);
    dateTime->hour   = read_big_u16(header->CreationDateTime + 6);
    dateTime->minute = read_big_u16(header->CreationDateTime + 8);
    dateTime->second = read_big_u16(header->CreationDateTime + 10);
}
