...
static float table_func_16(int i, int n, const void* ctx) {
    (void)n;
    return read_big_u16((const uint8_t*)ctx + 2 * i) * (1 / 65535.0f);
}

static float table_func_8(int i, int n, const void* ctx) {
    (void)n;
...
bool skcms_ApproximateCurve(const skcms_Curve* curve, skcms_TransferFunction* approx,
                            float* max_error) {
    // TODO: If !table_entries, just copy curve->parametric and set max_error to 0?
