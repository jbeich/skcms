...
static bool Matrix4x4d_isfinite(const skcms_Matrix4x4d* m) {
    for (int r = 0; r < 4; ++r)
    for (int c = 0; c < 4; ++c) {
        if (!isfinite(m->vals[r][c])) {
            return false;
...
bool skcms_Matrix4x4d_invert(const skcms_Matrix4x4d* src, skcms_Matrix4x4d* dst) {
    ...
    return Matrix4x4d_isfinite(dst);
}
...
skcms_Vector4d skcms_Matrix4x4d_Vector4d_mul(const skcms_Matrix4x4d* m, const skcms_Vector4d* v) {
    skcms_Vector4d dst = { { 0,0,0,0 } };
    for (int row = 0; row < 4; ++row) {
        dst.vals[row] = m->vals[row][0] * v->vals[0]
                      + m->vals[row][1] * v->vals[1]
                      + m->vals[row][2] * v->vals[2]
