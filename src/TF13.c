...
bool skcms_ApproximateCurve13(const skcms_Curve* curve, skcms_TF13* approx, float* max_error) {
    ...
    *max_error = 0;
    for (int i = 0; i < N; i++) {
        ...
        // Compare what bytes we'd choose for these floats, clamped, rounded, and scaled by 255.

        const uint8_t gbyte = (uint8_t)(255.0f * fmaxf_(0, fminf_(got , 1)) + 0.5f),
                      wbyte = (uint8_t)(255.0f * fmaxf_(0, fminf_(want, 1)) + 0.5f);
