...
static float eval_nonlinear(float x, const void* ctx) {
    const skcms_TransferFunction* tf = (const skcms_TransferFunction*)ctx;

    assert(tf->a * x     + tf->b > 0);
    assert(tf->a * tf->d + tf->b > 0);

    // (Notice that a large amount of this work is independent of x.)
    return powf_(tf->a * x     + tf->b, tf->g)
         - powf_(tf->a * tf->d + tf->b, tf->g)
         +       tf->c * tf->d + tf->f;
}
static void grad_nonlinear(float x, const float P[4], float dfdP[4]) {
...
bool skcms_ApproximateCurve(const skcms_Curve* curve,
    ...
    for (int t = 0; t < ARRAY_COUNT(kTolerances); t++) {
        ...
        if (L == N) {
            ...
            // Guess some initial values for a,b,g.
            // TODO: restore smarter g guess
        #if 0
            int mid = (start + N) / 2;
