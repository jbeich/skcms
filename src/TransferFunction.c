...
static float eval_nonlinear(float x, const void* ctx, const float P[4]) {
    ...
    // Pin to 0 to prevent powf_() from exploding.
    const float X = fmaxf_(a*x + b, 0),
                D = fmaxf_(a*d + b, 0);
    return powf_(X, g)
         - powf_(D, g)
         + c*d + f;
...
static bool fit_nonlinear(const skcms_Curve* curve, int start, int N, skcms_TransferFunction* tf) {
    ...
    for (int j = 0; j < 3/*TODO: tune*/; j++) {
        ...
        // This extra constraint that a >= 0 is not modeled in the optimization.
        tf->a = fmaxf_(tf->a, 0);
        // This extra constraint that ad+b >= 0 is not modeled in the optimization.
        if (tf->a * tf->d + tf->b < 0) {
            tf->b = -tf->a * tf->d;
        }
    }
...
bool skcms_ApproximateCurve(const skcms_Curve* curve,
    ...
    *max_error = INFINITY_;
    ...
    for (int t = 0; t < ARRAY_COUNT(kTolerances); t++) {
        skcms_TransferFunction tf = {0,0,0,0,0,0,0};
