...
static float tf_eval_nonlinear(const skcms_TransferFunction* fn, float x) {
...
static bool tf_gauss_newton_step_nonlinear(skcms_TableFunc* t, const void* ctx, int start, int n,
    ...
    for (int i = start; i < n; ++i) {
        ...
        float r = t(i, ctx) - tf_eval_nonlinear(fn, xi);
        LOG("r: %.25g\n", r);
    ...
    *error_Linfty_after = 0;
    for (int i = start; i < n; ++i) {
        float xi = i / (n - 1.0f);
        float error = fabsf_(t(i, ctx) - tf_eval_nonlinear(fn, xi));
