...
SI F gather_16(const uint8_t* p8, I32 ix) {
    const uint16_t* p;
    memcpy(&p, &p8, sizeof(p));
#if N == 1
    U16 v = (U16)p[2*ix] << 8 | (U16)p[2*ix+1];
#elif N == 4
    U16 v = (U16){ p[ix[0]], p[ix[1]], p[ix[2]], p[ix[3]] };
#elif N == 8
    U16 v = (U16){ p[ix[0]], p[ix[1]], p[ix[2]], p[ix[3]],
    ...
    return CAST(F, (v<<8)|(v>>8)) * (1/65535.0f);
}
