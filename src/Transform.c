...
static void unpremul(int i, void** ip, Context* ctx, F r, F g, F b, F a) {
    ...
    F scale = if_then_else(F1 / a < inf, F1 / a, F0);
    r *= scale;
    g *= scale;
    b *= scale;
    next_stage(i,ip,ctx, r,g,b,a);
...
bool skcms_Transform(const void*             src,
    ...
    switch (srcFmt >> 1) {
        ...
        case skcms_PixelFormat_RGB_101010x   >> 1: *ip++ = (void*)load_1010102;  break;
    ...
    if (srcAlpha == skcms_AlphaFormat_Opaque ||
        (srcFmt >> 1) == (skcms_PixelFormat_RGB_101010x >> 1)) {
    ...
    if (srcAlpha == skcms_AlphaFormat_PremulAsEncoded) {
        *ip++ = (void*)unpremul;
    }
    ...
    if (dstAlpha == skcms_AlphaFormat_PremulAsEncoded) {
        *ip++ = (void*)premul;
    }
    if (dstFmt & 1) {
        *ip++ = (void*)swap_rb;
    ...
    if (dstAlpha == skcms_AlphaFormat_Opaque ||
        (dstFmt >> 1) == (skcms_PixelFormat_RGB_101010x >> 1)) {
        *ip++ = (void*)force_opaque;
    }
    switch (dstFmt >> 1) {
        default: return false;
        ...
        case skcms_PixelFormat_RGB_101010x   >> 1: *ip++ = (void*)store_1010102;  break;
