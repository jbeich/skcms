...
// We define most helper functions using this macro, to enforce good code
// generation with the desired instruction set, but also work around what we
// think is a bug in GCC: when targeting 32-bit x86, GCC tends to pass U16 (4x
// uint16_t vector) function arguments in the MMX mm0 register, which seems to
// mess with unrelated code that later uses x87 FP instructions (MMX's mm0 is
// an alias for x87's st0 register).
//
// This is typically followed up by #define name NS(name_) to make usage nicer.
#if defined(__clang__) || defined(__GNUC__)
    #define HELPER(name) static inline ATTR __attribute__((always_inline)) NS(name ## _)
#else
    #define HELPER(name) static inline name ## _
#endif
...
I32 HELPER(to_fixed)(F f) {  return CAST(I32, f + 0.5f); }
...
// always (T) cast the result to the type you expect the result to be.
    ...
    F   HELPER(F_from_Half(U16 half)) { return vcvt_f32_f16(half); }
    U16 HELPER(Half_from_F(F      f)) { return vcvt_f16_f32(   f); }
#elif defined(__AVX512F__)
    F   HELPER(F_from_Half)(U16 half) { return (F)  _mm512_cvtph_ps((__m256i)half); }
    U16 HELPER(Half_from_F)(F f)      { return (U16)_mm512_cvtps_ph((__m512 )f,
                                                                     _MM_FROUND_CUR_DIRECTION ); }
#elif defined(USING_AVX_F16C)
    F   HELPER(F_from_Half)(U16 half) { return (F)  _mm256_cvtph_ps((__m128i)half); }
    U16 HELPER(Half_from_F)(F f)      { return (U16)_mm256_cvtps_ph((__m256 )f,
                                                                     _MM_FROUND_CUR_DIRECTION ); }
#else
    F HELPER(F_from_Half)(U16 half) {
...
// Swap high and low bytes of 16-bit lanes, converting between big-endian and little-endian.
    ...
    U16 HELPER(swap_endian_16)(U16 v) {
...
void HELPER(swap_endian_16x4)(U64* rgba) {
...
#if defined(USING_NEON)
    F HELPER(min_)(F x, F y) { return vminq_f32(x,y); }
    F HELPER(max_)(F x, F y) { return vmaxq_f32(x,y); }
#else
    F HELPER(min_)(F x, F y) { return (F)if_then_else(x > y, y, x); }
    F HELPER(max_)(F x, F y) { return (F)if_then_else(x < y, y, x); }
#endif
...
F HELPER(approx_pow)(F x, float y) {
...
// Return tf(x).
F HELPER(apply_transfer_function)(const skcms_TransferFunction* tf, F x) {
...
U8 HELPER(gather_8)(const uint8_t* p, I32 ix) {
#if N == 1
    U8 v = p[ix];
#elif N == 4
...
uint16_t HELPER(load_16)(const uint8_t* p, int ix) {
    uint16_t v;
...
U16 HELPER(gather_16)(const uint8_t* p, I32 ix) {
#if N == 1
    U16 v = load_16(p,ix);
#elif N == 4
...
#define gather_16 NS(gather_16_)
    ...
    uint32_t HELPER(load_24_32)(const uint8_t* p, int ix) {
        uint32_t v;
        small_memcpy(&v, p + 3*ix, 4);
        return v;
    }
    uint64_t HELPER(load_48_64)(const uint8_t* p, int ix) {
        uint64_t v;
...
U32 HELPER(gather_24)(const uint8_t* p, I32 ix) {
...
void HELPER(gather_48)(const uint8_t* p, I32 ix, U64* v) {
...
F HELPER(table8)(const skcms_Curve* curve, F v) {
...
#define table8 NS(table8_)

F HELPER(table16)(const skcms_Curve* curve, F v) {
    // All just as in table8() until the gathers.
    F ix = max_(F0, min_(v, F1)) * (float)(curve->table_entries - 1);
...
#define table16 NS(table16_)

// Color lookup tables, by input dimension and bit depth.
void HELPER(clut_0_8)(const skcms_A2B* a2b, I32 ix, I32 stride, F* r, F* g, F* b, F a) {
...
void HELPER(clut_0_16)(const skcms_A2B* a2b, I32 ix, I32 stride, F* r, F* g, F* b, F a) {
    U64 rgb;
...
#define DEF_CLUT(I,J,B)                                                                            \
    void HELPER(clut_##I##_##B)(const skcms_A2B* a2b, I32 ix, I32 stride, F* r, F* g, F* b, F a) { \
...
static void NS(exec_ops)(const Op* ops, const void** args,
    ...
    while (true) {
        switch (*ops++) {
            ...
            case Op_table8_r: { r = table8 (*args++, r); } break;
            case Op_table8_g: { g = table8 (*args++, g); } break;
            case Op_table8_b: { b = table8 (*args++, b); } break;
            case Op_table8_a: { a = table8 (*args++, a); } break;

            case Op_table16_r:{ r = table16(*args++, r); } break;
            case Op_table16_g:{ g = table16(*args++, g); } break;
            case Op_table16_b:{ b = table16(*args++, b); } break;
            case Op_table16_a:{ a = table16(*args++, a); } break;
