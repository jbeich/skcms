...
static void exec_ops(const Op* ops, const void** args,
    ...
    // Notice, from here on down the store_ ops all return, ending the loop.
            ...
            case Op_store_8888: {
                U32 R = cast<U32>(to_fixed(r * 255)),
                    G = cast<U32>(to_fixed(g * 255)),
                    B = cast<U32>(to_fixed(b * 255)),
                    A = cast<U32>(to_fixed(a * 255));
                store(dst + 4*i, R <<  0
                               | G <<  8
                               | B << 16
                               | A << 24);
            ...
            case Op_store_1010102: {
                U32 R = cast<U32>(to_fixed(r * 1023)),
                    G = cast<U32>(to_fixed(g * 1023)),
                    B = cast<U32>(to_fixed(b * 1023)),
                    A = cast<U32>(to_fixed(a *    3));
                store(dst + 4*i, R <<  0
                               | G << 10
                               | B << 20
                               | A << 30);
