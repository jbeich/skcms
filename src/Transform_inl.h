...
SI ATTR void NS(gather_48_)(const uint8_t* p, I32 ix, U64* v) {
    ...
    *v = (U64){load_48_64(p,ix[0]), load_48_64(p,ix[1]), load_48_64(p,ix[2]), load_48_64(p,ix[3]),
    ...
    typedef int64_t __attribute__((vector_size(32))) Half_I64;
    ...
    #if defined(__clang__)
        Half_I64 lo = (Half_I64)__builtin_ia32_gatherd_q256(zero, p8, 6*ix_lo, mask, 1),
                 hi = (Half_I64)__builtin_ia32_gatherd_q256(zero, p8, 6*ix_hi, mask, 1);
    #elif defined(__GNUC__)
        Half_I64 lo = (Half_I64)__builtin_ia32_gathersiv4di(zero, p8, 6*ix_lo, mask, 1),
                 hi = (Half_I64)__builtin_ia32_gathersiv4di(zero, p8, 6*ix_hi, mask, 1);
    #endif
