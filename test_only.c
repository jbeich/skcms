...
static void dump_approx_transfer_function(FILE* fp, const skcms_TransferFunction* tf,
    ...
    if (for_unit_test) {
        ...
        // test mode, we just indicate that an approximation was found. Real testing occurs in
        // test_Parse (via round-tripping).
        fprintf(fp, "  ~= : %.4g, %.4g, %.4g, %.4g, %.4g, %.4g, %.4g  (Max error: %.2g)\n",
                (double)tf->g, (double)tf->a, (double)tf->b, (double)tf->c,
                (double)tf->d, (double)tf->e, (double)tf->f, (double)max_error);
//        fprintf(fp, "     : Approximate curve found\n");
    } else {
