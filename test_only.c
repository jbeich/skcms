...
static void dump_approx_transfer_function(FILE* fp, const skcms_TransferFunction* tf,
    ...
    if (for_unit_test) {
        ...
        // test mode, we print at reduced precision. This test just ensures that the values are
        // somewhat close. More thorough testing occurs in test_Parse (via round-tripping).
        fprintf(fp, "  ~= : %.1g, %.2g, %.2g, %.2g, %.2g, %.2g %.2g  (Max error: %.2g)\n",
                (double)tf->g, (double)tf->a, (double)tf->b, (double)tf->c,
