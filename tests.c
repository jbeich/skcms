...
static void test_HLG_CICP(void) {
    #define NUM_TESTS 3
    // Represent using CICP.
    float signal_values[NUM_TESTS] = {0.0f, 0.75f, 1.0f};
    float linear_values[NUM_TESTS] = {0.0f, 1.0f, 1000.0f / 203.0f};
...
static void test_CLUT_PageBoundary(void) {
    ...
    memset((char*)ptr + pagesize, 0, pagesize);
...
static void test_CLUT_PageBoundary2(void) {
    ...
    // This profile has a 16-bit 3-channel CLUT and nothing after the table.
    // It's 2x2x2, so when skcms would go to read entry 7 (the last one), it would
    // overflow onto the unmapped page by reading 8 bytes when there's only 6 to read.
    ...
    // Same as fuzz_iccprofile_transform. We shouldn't see an overread.
