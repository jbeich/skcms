...
static void check_curves(uint32_t num_channels, const skcms_Curve* ref, const skcms_Curve* curves) {
    ...
    for (uint32_t i = 0; i < num_channels; ++i) {
        expect(fabsf(ref[i].parametric.g - curves[i].parametric.g) < kTol);
        expect(fabsf(ref[i].parametric.a - curves[i].parametric.a) < kTol);
        expect(fabsf(ref[i].parametric.b - curves[i].parametric.b) < kTol);
        expect(fabsf(ref[i].parametric.c - curves[i].parametric.c) < kTol);
        expect(fabsf(ref[i].parametric.d - curves[i].parametric.d) < kTol);
        expect(fabsf(ref[i].parametric.e - curves[i].parametric.e) < kTol);
        expect(fabsf(ref[i].parametric.f - curves[i].parametric.f) < kTol);
...
static void check_a2b(const skcms_A2B* ref, const skcms_A2B* a2b) {
    ...
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 4; ++c) {
            expect(fabsf(ref->matrix.vals[r][c] - a2b->matrix.vals[r][c]) < 1E-5f);
        }
    }
