"""
Based on
https://skia.googlesource.com/skia/+/773abacc15810af28f45ea234efdf6a96b5f16cf/toolchain/download_mac_toolchain.bzl.
...
def _download_mac_toolchain_impl(ctx):
    # Abort if the host OS is not macOS. This rule fails on non-macOS platforms because it depends
    # on the xcode-select command, which is only available on macOS.
    #
    # Note that as of 2023-10-25, this is not necessary in the Skia repository, from which this
    # rule was copied almost verbatim. That is, Bazel somehow knows not to execute this rule. It
    # might be due to the fact that this and Skia use different Bazel versions.
    if "mac" not in ctx.os.name:
        # Bazel fails if a repository rule produces no outputs, so we create a placeholder file.
        ctx.file("empty.txt", "Skipping incompatible toolchain.", executable=False)
        return
