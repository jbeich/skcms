"""
Based on
https://skia.googlesource.com/skia/+/0d4fcf388a6f9318e2a54fa85fddf3396d521767/toolchain/mac_toolchain_config.bzl.
...
def _make_default_flags():
    ...
    return [feature(
        ...
        enabled = True,
        flag_sets = [
            # -Weverything (from `cxx_compile_includes`) must come before any -Wno_xxx flags in the
            # flag sets below. The goal is to first enable all warnings, and then to disable the
            # ones we do not need.
